...
contract NFT is ERC721Enumerable, Ownable, VRFConsumerBaseV2 {
    ...
    function requestRandomWords() public returns(uint8) {
        // Will revert if subscription is not set and funded.
        uint8 _requestId = uint8(COORDINATOR.requestRandomWords(
            keyHash,
            s_subscriptionId,
    ...
    function findUnminted() internal returns(uint16) {
        uint16 supply = uint16(totalSupply());
        uint16 index = requestRandomWords() % (maxSupply - supply);
        uint16 chosenNft = mints[index];
        uint16 swapNft = mints[maxSupply - supply - 1];
